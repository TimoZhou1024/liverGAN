...
from liqa_mrgan3d.models.pix2pix_3d import Generator3D, PatchDiscriminator3D, weights_init_normal_3d
from liqa_mrgan3d.trainer.losses import GANLoss, GaussianBlur3D
...
class LiQA3DTrainer:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.output_dir = ensure_dir(config.get("save_root", "outputs/liqa_3d_patch"))
        self.checkpoint_dir = ensure_dir(Path(self.output_dir) / "checkpoints")
        self.log_dir = ensure_dir(Path(self.output_dir) / "logs")

        self.input_nc = int(config.get("input_nc", len(config.get("input_modalities", []))))
        ...
        self.net_g = Generator3D(
            input_nc=self.input_nc,
            output_nc=self.output_nc,
            base_channels=int(config.get("g_base_channels", 16)),
        ).to(self.device)
        self.net_d = PatchDiscriminator3D(
            input_nc=self.condition_channels_for_d + self.output_nc,
            ndf=int(config.get("d_base_channels", 16)),
            n_layers=int(config.get("d_layers", 3)),
        ).to(self.device)
        self.net_g.apply(weights_init_normal_3d)
        self.net_d.apply(weights_init_normal_3d)
    ...
    def train(self) -> None:
        ...
        for epoch in range(start_epoch, n_epochs):
            ...
            for batch in progress:
                ...
                if global_step % int(self.config.get("log_interval", 10)) == 0:
